...
public class Param {
	...
	public Param Clone(Dictionary<object, object> clonedObjects)
	{
		if (clonedObjects.ContainsKey(this))
			return (Param)clonedObjects[this];

		var clone = new Param(this.name, this.value)
		{
			reduceable = this.reduceable,
			changed = this.changed,
			IsDrag = this.IsDrag
		};
		clonedObjects[this] = clone;

		if (exp != null)
			clone.exp = exp.Clone(clonedObjects);
...
public class Exp
{
	...
	static public Exp Sin(Exp x)
	{
		return new Exp(Op.Sin, x, null);
	}

	static public Exp Cos(Exp x)
	{
		return new Exp(Op.Cos, x, null);
	}

	static public Exp ACos(Exp x)
	{
		return new Exp(Op.ACos, x, null);
	}

	static public Exp ASin(Exp x)
	{
		return new Exp(Op.ASin, x, null);
	}

	static public Exp Sqrt(Exp x)
	{
		return new Exp(Op.Sqrt, x, null);
	}

	static public Exp Sqr(Exp x)
	{
		return new Exp(Op.Sqr, x, null);
	}

	static public Exp Abs(Exp x)
	{
		return new Exp(Op.Abs, x, null);
	}

	static public Exp Sign(Exp x)
	{
		return new Exp(Op.Sign, x, null);
	}

	static public Exp Atan2(Exp x, Exp y)
	{
		return new Exp(Op.Atan2, x, y);
	}

	static public Exp Expo(Exp x)
	{
		return new Exp(Op.Exp, x, null);
	}

	static public Exp Sinh(Exp x)
	{
		return new Exp(Op.Sinh, x, null);
	}

	static public Exp Cosh(Exp x)
	{
		return new Exp(Op.Cosh, x, null);
	}

	static public Exp SFres(Exp x)
	{
		return new Exp(Op.SFres, x, null);
	}

	static public Exp CFres(Exp x)
	{
	...
	public Exp Clone(Dictionary<object, object> clonedObjects)
	{
		if (clonedObjects.ContainsKey(this))
			return (Exp)clonedObjects[this];
		...
		if (a != null)
			clone.a = a.Clone(clonedObjects);
		if (b != null)
			clone.b = b.Clone(clonedObjects);
		if (param != null)
			clone.param = param.Clone(clonedObjects);
